...
public class VendorCustomRepositoryImpl implements VendorCustomRepository {
    ...
    @Override
    public List<Vendor> getFeaturedVendorList(CoordinatesDTO coordinatesDTO, int featuredVendorDistance) {
        Map<String, Object> paramMap = new HashMap<>();
        StringBuilder sqlQuery;
        boolean isLocation =  coordinatesDTO.getLatitude() != null &&
                coordinatesDTO.getLongitude() != null &&
                coordinatesDTO.getLatitude().toString() != "" &&
                coordinatesDTO.getLongitude().toString() != "";
        if (isLocation) {
            sqlQuery = new StringBuilder(
                    "SELECT v.id,v.email,v.first_name_english,v.last_name_english,v.store_name_english,v.store_image_name,v.store_detail_image_name,v.featured_image_name,v.latitude,v.longitude,v.rating,v.no_of_rating,v.is_featured,v.accepts,v.phone_number,v.opening_hours_from,v.opening_hours_to,v.delivery_type,v.payment_method,v.minimum_order_amt,v.store_phone_number,v.first_name_arabic,v.last_name_arabic,v.store_name_arabic, (( 3959 * acos( cos( radians(:customerLatitude) ) * cos( radians(latitude) ) * cos( radians(longitude) "
                            + "- radians(:customerLongitude) ) + sin( radians(:customerLatitude) ) * sin( radians(latitude) ) ) )*1.60934) AS distance "
                            + "FROM vendor v WHERE ");
            sqlQuery.append(" (( 3959 * acos( cos( radians(:customerLatitude) ) * cos( radians(latitude) ) * cos( radians(longitude) - radians(:customerLongitude) ) + sin( radians(:customerLatitude) ) * sin( radians(latitude) ) ) )*1.60934) <=:featuredVendorDistance and ");
            paramMap.put("featuredVendorDistance", featuredVendorDistance);
        } else {
            sqlQuery = new StringBuilder(
                    "SELECT v.id,v.email,v.first_name_english,v.last_name_english,v.store_name_english,v.store_image_name,v.store_detail_image_name,v.featured_image_name,v.latitude,v.longitude,v.rating,v.no_of_rating,v.is_featured,v.accepts,v.phone_number,v.opening_hours_from,v.opening_hours_to,v.delivery_type,v.payment_method,v.minimum_order_amt,v.store_phone_number,v.first_name_arabic,v.last_name_arabic,v.store_name_arabic "
                            + "FROM vendor v WHERE ");
        }
        sqlQuery.append(" v.active = true and v.status = '" + VendorStatus.ACTIVE.getStatusValue()
                + "' and v.is_order_service_enable = true and v.profile_completed = true and v.is_featured = true");
        addConditionsForVendorList(coordinatesDTO, sqlQuery, paramMap);
        Query q = entityManager.createNativeQuery(sqlQuery.toString());
        paramMap.entrySet().forEach(p -> q.setParameter(p.getKey(), p.getValue()));
        Object obj = q.getResultList();
        List<Object[]> responseObjList = (List<Object[]>) obj;
    ...
    private void addConditionsForVendorList(final CoordinatesDTO coordinatesDTO, final StringBuilder sqlQuery, final Map<String, Object> paramMap) {
        if (coordinatesDTO.getLatitude() != null) {
            paramMap.put("customerLatitude", coordinatesDTO.getLatitude());
        }
        if (coordinatesDTO.getLongitude() != null) {
            paramMap.put("customerLongitude", coordinatesDTO.getLongitude());
            sqlQuery.append(" order by distance asc ");
        }
    }
}
