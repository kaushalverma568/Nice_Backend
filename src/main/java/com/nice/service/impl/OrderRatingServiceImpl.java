...
public class OrderRatingServiceImpl implements OrderRatingService {
	...
	public OrderRatingResponseDTO addOrderRating(final OrderRatingDTO orderRatingDTO) throws NotFoundException, ValidationException {
		 ...
		 */
		if (orderRating.getQuestion4Rating() != null && orderRating.getQuestion5Rating() != null) {
			orderRating.setAvgOrderRating(Math.round((orderRating.getQuestion1Rating() + orderRating.getQuestion2Rating() + orderRating.getQuestion3Rating()
					+ orderRating.getQuestion4Rating() + orderRating.getQuestion5Rating()) / 5.0 * 100.0) / 100.0);
		} else {
			orderRating.setAvgOrderRating(
					Math.round((orderRating.getQuestion1Rating() + orderRating.getQuestion2Rating() + orderRating.getQuestion3Rating()) / 3.0 * 100.0) / 100.0);
		}
		/**
		 * order rating save
	...
	private void validateOrderRating(final OrderRatingDTO orderRatingDTO) throws NotFoundException, ValidationException {
		Orders order = orderService.getOrderById(orderRatingDTO.getOrderId());
		if (order.getDeliveryType().equalsIgnoreCase(DeliveryType.DELIVERY.name()) || order.getDeliveryType().equalsIgnoreCase(DeliveryType.BOTH.name())) {
	...
	public void changeStatus(final Long orderRatingId, final Boolean active) throws ValidationException, NotFoundException {
		...
		if (active == null) {
			...
			throw new ValidationException(
					messageByLocaleService.getMessage(Boolean.TRUE.equals(active) ? "order.rating.active" : "order.rating.deactive", null));
		} else {
	...
	@Override
	public Page<OrderRating> getList(final Integer pageNumber, final Integer pageSize, final Boolean activeRecords, final String searchKeyWord) {
	...
	private void deliveryBoyRatingCalculation(final OrderRating orderRating) throws NotFoundException {
	...
	private void vendorRatingVCalculation(final OrderRating orderRating) throws NotFoundException {
	...
	@Override
	public OrderRatingResponseDTO getOrderRatingbyOrderId(final Long orderId) throws NotFoundException {
