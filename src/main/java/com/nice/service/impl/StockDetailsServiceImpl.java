...
public class StockDetailsServiceImpl implements StockDetailsService {
	...
	public void moveQtyToExpiredState(final Date runDate) {
		...
		try {
			schedulerDetailsService.updateSchedulerDate(Constant.EXPIRE_STOCK_SCHEDULER, runDate);
	...
	@Override
	public Page<ProductVariantResponseDTO> getLowStockProduct(final Integer pageNumber, final Integer pageSize) throws ValidationException, NotFoundException {
		UserLogin userLogin = ((UserAwareUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUser();
		if (UserType.VENDOR.name().equals(userLogin.getEntityType())) {
			Pageable pageable = PageRequest.of(pageNumber - 1, pageSize);
			List<BigInteger> productVariantIdList = stockDetailsCustomRepository.getLowStockProductDetails(userLogin.getEntityId(), (int) pageable.getOffset(),
					pageSize);
	...
	@Override
	public Page<StockDetailsDTO> getExpireStockDetails(final Integer pageNumber, final Integer pageSize) throws ValidationException, NotFoundException {
		UserLogin userLogin = ((UserAwareUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUser();
		if (UserType.VENDOR.name().equals(userLogin.getEntityType())) {
			Pageable pageable = PageRequest.of(pageNumber - 1, pageSize);
			List<StockDetailsDTO> stockDetailsDTOs = new ArrayList<>();
			List<StockDetails> stockDetailsList = stockDetailsCustomRepository.getExpiredStockDetails(userLogin.getEntityId(), (int) pageable.getOffset(),
					pageSize);
